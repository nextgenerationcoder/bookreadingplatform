...
// LessonPlayer mic button. Groq is the only provider (the self-hosted
// Whisper container this used to default to was removed - it cost ~1.4GB
// of RAM on the VPS for a feature Groq already covers, faster, on its own
// hardware) - so there's no keyless default anymore; the mic button just
// doesn't work until an account adds its own Groq key here.
router.get('/asr', (req, res) => {
  const row = db.prepare('SELECT asr_provider, asr_api_key_enc FROM users WHERE id = ?').get(req.userId);
  res.json({ provider: row?.asr_api_key_enc ? row.asr_provider || 'groq' : null, configured: Boolean(row?.asr_api_key_enc) });
});
...
router.delete('/asr', (req, res) => {
  ...
  res.json({ provider: null, configured: false });
});
